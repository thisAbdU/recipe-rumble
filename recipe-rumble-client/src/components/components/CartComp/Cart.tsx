...
import padTad from '@/public/assets/pad_tad.jpeg';
import sushi from '@/public/assets/sushi.jpg';
...
const Cart = () => {
    ...
    const [items, setItems] = useState([
        { id: 1, name: 'Sushi', price: 8, quantity: 1, image: sushi },
        { id: 2, name: 'Pad Thai ', price: 10, quantity: 2, image: padTad }
    ]);
    ...
    return (
        ...
        <div className='w-[80%] mx-auto py-6'>
            <IoArrowBackCircle size={30} className='text-orange-500 cursor-pointer' onClick={() => router.push("/recipes")} />
        </div>
                    ...
                    <div key={item.id} className="flex flex-col md:flex-row items-center justify-between shadow-lg p-4 rounded-lg w-[80%] mx-auto">
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="flex flex-col">
                                ...
                                <p className="md:hidden">Price: {item.price} $</p>
                                <p className="md:hidden">Subtotal: {calculateSubtotal(item.price, item.quantity)} $</p>
                            </div>
                        </div>
                        ...
                        <input 
                            ...
                            min="1"
                        />
                        <p className="hidden md:block">{calculateSubtotal(item.price, item.quantity)} $</p>
                    </div>
                ))}
            </div>
            <div className="flex flex-col md:flex-row justify-between items-center mt-8 space-y-4 md:space-y-0 w-full md:w-[80%] mx-auto">
            <button className="border p-3 bg-orange-600 text-center text-white hover:bg-orange-700 rounded-lg" onClick={() => router.push("/recipes")}>Return to Shop</button>
            <button className="border p-3 bg-orange-600 text-center text-white hover:bg-orange-700 rounded-lg">Update Cart</button>
                </div>
    ...
    <button className="border p-2 bg-orange-600 text-center text-white hover:bg-orange-700 rounded-lg w-full md:w-auto">Apply Coupon</button>
        </div>
        <div className="mt-8 p-4 rounded border border-black w-full md:w-[40%] mx-auto space-y-3">
            ...
            <button className="w-full p-2 rounded-lg text-white mt-4 bg-orange-600 hover:bg-orange-700 text-center" onClick={() => router.push("/pages/checkout")}>Proceed to Checkout</button>
        </div>
        </div>
    );
};
